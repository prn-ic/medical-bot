...
async def get_data_by_symptom(callback: types.CallbackQuery):
    index = callback.data.replace('select_symptom ', '').split('|')[1]
    cause = get_symptom_causes(callback.data
                               .replace('select_symptom ', '').split('|')[0])[int(index)]

    keyboard = generate_symptom_keyboard(cause.symptom.id, int(index) + 1, score=0)
...
async def respond_answer(callback: types.CallbackQuery, answer_type: str):
    try:
        index = callback.data.replace('yes_cause ', '').split('|')[1]
        score = callback.data.replace('no_cause ', '').split('|')[2]

        cause = get_symptom_causes(callback.data
                                   .replace(f'{answer_type}_cause ', '').split('|')[0])[int(index)]
